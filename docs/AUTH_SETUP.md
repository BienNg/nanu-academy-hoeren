...
3. Open the linked Supabase project → **SQL Editor** → run `supabase/user_progress.sql` (re-run after pulls — the script is idempotent and adds `email` / `name` / `last_login_at` / `level_access` / `class_name` used by `/admin`)
